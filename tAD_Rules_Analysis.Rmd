...
P1=ggplot(RulesFData)+
  geom_boxplot(aes(x=set,y=SlopeFinal200))+
  theme_classic()+
  ylab("Slope")+
  xlab("")+
  geom_hline(yintercept = 0,color="red",linewidth=1.2)+  
  theme(axis.text.x = element_blank(),axis.text = element_text(size=14),axis.title.y = element_text(size=16,vjust = -4))
...
ggplot(AUCdf,aes(AUC,Library,group = factor(RulesVersion,levels = c("New","Old"))))+
  ...
  scale_fill_manual(name = "Rules\nVersion",values = c("red3","blue3"),breaks = c("Old","New"))+
...
P2=ggplot(WDdf[WDdf$WCount!=14,],aes(x=WCount,y=Slope,group=WCount))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
  geom_point(size=2,aes(x=WCount,y=Slope),data = WDdf[WDdf$WCount==14,])+
...
P3=ggplot(WDdf[WDdf$DCount!=13 & WDdf$DCount!=14,],aes(x=DCount,y=Slope,group=DCount))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
  geom_point(size=2,aes(x=DCount,y=Slope),data = WDdf[WDdf$DCount==13 | WDdf$DCount==14,])+
...
P=wrap_plots(A=P1,B=P2,C=P3,design = layout,widths = c(5,4.5,5.5),heights = c(5,0.5,4.5))
ggsave("Figures/WandDcount.png",P,width = 10,height = 10)

#Balanced WD graph
BalWDdf=filter(WDdf,WCount==DCount)
ggplot(BalWDdf,aes(x=factor(WCount),y=Slope))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
...
P1=ggplot(WDRdf[WDRdf$RCount!=10,],aes(x=RCount,y=Slope,group=RCount))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
...
ggsave("Figures/WDRcharge.png",P,width = 11,height = 5)          
...
P1=filter(ProlineFdata, Wcount==2)%>%
ggplot(aes(x=as.factor(Dcount),y=SlopeFinal200))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+  
  theme_classic()+
...
P2=filter(ProlineFdata, Wcount==3)%>%
ggplot(aes(x=as.factor(Dcount),y=SlopeFinal200))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+  
  theme_classic()+
...
P1=ggplot(Balancedf[Balancedf$BalDev<8,],aes(x=factor(BalDev),y=Slope))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
  theme_classic()+
  ylab("Slope")+
...
P2=ggplot(Balancedf,aes(x=factor(PosDev),y=Slope))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
  theme_classic()+
  ylab("Slope")+
...
P3=Balancedf%>%filter(AroCluster<7&AroCluster>1)%>%
ggplot(aes(x=factor(AroCluster),y=Slope,fill=factor(Flanked)))+
  ...
  scale_fill_manual(name="Flanked?",values = c("red3","blue3"),labels = c("No","Yes"))+
  theme_classic()+
  ylab("Slope")+
...
P1=ggplot(Set32varFdata[Set32varFdata$set %in% ReorderSets[1:9],],aes(x=set,y=SlopeFinal200))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+ 
  theme_classic()+
  ylab("Slope")+
...
#G substitutions
P2=ggplot(Set32varFdata[Set32varFdata$set %in% ReorderSets[c(1,10:20)],],aes(x=fct_reorder(set,desc(SlopeFinal200)),y=SlopeFinal200))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+ 
  theme_classic()+
  ylab("Slope")+
...
P3=ggplot(HydrophobicDF[HydrophobicDF$Count!=1 & HydrophobicDF$index!=7883,],aes(x=as.factor(Count),y=SlopeFinal200))+
  geom_boxplot(alpha=0.8,linewidth=1,outlier.color = NA)+
  geom_jitter(size=1.1,width = 0.3,height = 0)+
...
P4=ggplot(HydrophobicDF,aes(x=RuleCluster,y=SlopeFinal200,color = set))+
